...
/**
 * Reusable utility to send an email using the configured SMTP server.
 * Returns true if sent successfully, false if SMTP is not configured.
 */
export async function sendEmail(to: string, subject: string, html: string, text?: string) {
  const transporter = getTransporter()
  if (!transporter) {
    console.log(`[SMTP Skip] SMTP not configured. Attempted email to: ${to}, Subject: ${subject}`)
    return false
  }
...
`
  try {
    ...
    if (recipient.email) {
      ...
      const sent = await sendEmail(recipient.email, title, html, content)
      if (sent) {
