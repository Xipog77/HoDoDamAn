# Kiến trúc Hệ thống Gia Phả Platform

Tài liệu này mô tả chi tiết về cấu trúc, ngăn xếp công nghệ, luồng xác thực và mô hình dữ liệu của nền tảng Gia Phả.
...
src/
  ...
  routes/
    ...
    admin/persons.tsx            # Quản lý hồ sơ gia phả (TipTap editor, upload ảnh)
    admin/tree.tsx               # Quản lý quan hệ cây gia phả (gán cha/mẹ)
...
- **`users`**: Tài khoản đăng nhập. Luồng trạng thái: `PENDING` → `ACTIVE`.
  ...
  - Cột `personId` liên kết tài khoản với hồ sơ trong cây gia phả.
...
3. **Liên kết**: Quản trị viên gắn tài khoản với một hồ sơ trong cây gia phả (`personId`).
...
- **PostgreSQL duy nhất**: Cơ sở dữ liệu quan hệ đảm bảo tính toàn vẹn dữ liệu (FK, unique, NOT NULL) rất phù hợp cho gia phả. Cột `jsonb` cung cấp tính linh hoạt tương đương MongoDB khi cần. Với quy mô 300+ hồ sơ, PostgreSQL xử lý dễ dàng (thiết kế cho hàng triệu rows).
